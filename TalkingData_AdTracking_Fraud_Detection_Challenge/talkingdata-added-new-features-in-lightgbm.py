...
def do_next_Click(df, agg_suffix='nextClick', agg_type='float32'):
    ...
    for spec in GROUP_BY_NEXT_CLICKS:
        ...
        print(">> Group by {}, and saving time to {} in: {}".format(spec['groupby'], agg_suffix, new_feature))
...
def do_prev_Click(df, agg_suffix='prevClick', agg_type='float32'):
    ...
    print(">> \nExtracting {} time calculation feature...\n".format(agg_suffix))
    ...
    for spec in GROUP_BY_NEXT_CLICKS:
        ...
        print(">> Grouping by {}, and saving time to {} in: {}".format(agg_suffix, spec['groupby'], new_feature))
        df[new_feature] = (df.click_time - df[all_features].groupby(spec['groupby']).click_time.shift(+1)).dt.seconds.astype(agg_type)
        predictors.append(new_feature)
        gc.collect()
    return (df)

def do_count(df, group_cols, agg_type='uint16', show_max=False, show_ag=True):
...
def DO(frm, to, fileno):
    ...
    len_train = len(train_df)  # 训练集样本数
    train_df = train_df.append(test_df)

    del test_df

    gc.collect()
    # 从特征列click_time中提取新列hour、day
