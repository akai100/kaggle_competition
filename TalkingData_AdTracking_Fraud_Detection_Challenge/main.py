...
print('读取数据训练数据...')
train_df = pd.read_csv(path+"train.csv", nrows=35000, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

print('读取测试数据...')
test_df = pd.read_csv(path+"test.csv", dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])

len_train = len(train_df) # 训练集大小
...
print('提取新特征...')
train_df['hour'] = pd.to_datetime(train_df.click_time).dt.hour.astype('uint8')  # 点击时间的小时
train_df['day'] = pd.to_datetime(train_df.click_time).dt.day.astype('uint8')   # 点击时间所在天
gc.collect()

print('grouping by ip-day-hour combination...')
# 对同一个Ip在相同的时间点击移动广告商的次数
