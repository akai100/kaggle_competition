...
def fit_batch(clf, X, y, w):
    clf.partial_fit(X, y, sample_weight=2)

def predict_batch(clf, X):
    return clf.predict(X)

def evaluate_batch(clf, X, y, rcount):
    auc = roc_auc_score(y, predict_batch(clf, X))  # 计算auc
    global mean_auc
    if mean_auc == 0:
        mean_auc = auc
...
def ad_add_uniques(df, cols, tag="_unique"):
...
def df2csr(wb, df, pick_hours=None):
    ...
    with timer("Adding counts"):
        ...
        df = df_add_counts(df, ['ip', 'channel'])
    ...
    with timer("Log-bining features"):
        for fea in ['ip_day_hour_count', 'ip_app_count', 'ip_app_os_count',
